/*This Version is supposed to have a faster thr scan, due to reduced waiting times after each digi Pot adjustment.
 * In void Setup() there exists a new variable deltime which controls the delays in muS
*/
#include <AD5144.h>
#include <FreqCount.h>
//#include <RTClib.h>
...
#define SCINTI_READOUT_HW_VERSION 2
  ...
  //Define Control for Multiplexer choosing channel/coinc
  // Coin: Pin1 High, Pin2 High;; CH1: Pin1 High, P2 Low;; CH2: P1 Low, P2 High
 ...
 */
float offset[signal_channels] = {0, 0};
float gain[signal_channels] = {1, 1};
...
//
//       Serial.print("Step ");
//      Serial.print(spectrum0.i);
//      Serial.print(",  MAX: ");
//      Serial.println(spectrum0.max);
  ...
  if (scanning_channel == 1) {
    set_threshold(0, 255);
    digitalWrite(controlPin1, LOW); delay(deltime);
    digitalWrite(controlPin2, HIGH); delay(deltime);
  } else {
    set_threshold(1, 255);
    digitalWrite(controlPin2, LOW); delay(deltime);
    digitalWrite(controlPin1, HIGH); delay(deltime);
  }
...
int set_pe_threshold(const size_t channel, const double photoelectrons) {
  ...
  /*  Serial.print("thr chan");
    Serial.print(channel);
    Serial.print("  =");
    Serial.println(thr*1000);
*/
  if(thr < -0.5 || thr > 255) {
    return -1;
  }
...
void print_interrupts() {
  ...
  Serial.print(effective_thr(0)+1);
  Serial.print(" ");
  Serial.print(effective_thr(1)+1);
  Serial.print("  ");
...
void loop_scanning_single() {
  ...
  if (FreqCount.available()) {
    ...
    spectrum0.freq_err[0] = (1000 / (double) integration_time ) * sqrt(counts);
    print_interrupts();
